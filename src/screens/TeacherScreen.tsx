...
export function TeacherScreen() {
  ...
  const questions = selectedSet
    ...
    : [];
  const teachers = Object.entries(allUsers).filter(
    ([, u]) => u.role === "teacher",
  );
  ...
  const saveSet = async () => {
    ...
    try {
      if (editingId) {
        await api.updateKnowledgeSet(editingId, {
          grade: setForm.grade,
          topic: setForm.topic,
        });
        setSets((p) => ({
  ...
  const promoteTeacher = async () => {
    ...
    if (!entry) {
      setTeacherMsg("Không tìm thấy người dùng này.");
      return;
    }
    const [targetUid, targetUser] = entry;
    if (targetUser.role === "teacher") {
      setTeacherMsg("Người này đã là giáo viên.");
      return;
    }
    setSaving(true);
    try {
      ...
      setAllUsers((p) => ({
        ...p,
        [targetUid]: { ...p[targetUid], role: "teacher" },
      }));
      setNewTeacherName("");
      setModal("none");
      setTeacherMsg("");
  ...
  const confirmDelTeacher = (uid: string) => {
    setPDI(uid);
    setModal("del-teacher");
  };
  const demoteTeacher = async () => {
    ...
    try {
      ...
      setAllUsers((p) => ({
        ...p,
        [pendingDeleteId]: { ...p[pendingDeleteId], role: "student" },
      }));
      setModal("none");
    } finally {
      setSaving(false);
        ...
        {!(topTab === "questions" && qSub === "detail") && (
          ...
          >
            {(
              ...
              ] as { id: TopTab; icon: React.ReactNode; label: string }[]
            ).map((t) => (
              <motion.button
                key={t.id}
                onPointerDown={() => {
                ...
                style={{
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  color: topTab === t.id ? "#3E2000" : "#7D5A2C",
                }}
              >
                {t.icon}
                {t.label}
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                {setsLoading ? (
                  ...
                  <LoadMsg msg="Chưa có bộ câu hỏi nào. Nhấn + để thêm." />
                ) : (
          ...
          {topTab === "questions" && qSub === "detail" && (
            ...
            >
              ...
              >
                {questions.length === 0 ? (
                  <LoadMsg msg="Chưa có câu hỏi. Nhấn + để thêm." />
                ) : (
                  questions.map(([id, q], i) => {
                    const dm = DIFF_META[q.difficulty];
                    const validAnswers = ANS_KEYS.filter((k) =>
                      q.answers[k]?.trim(),
                    );
                    return (
                      <motion.div
                            ...
                            >
                              ...
                              {validAnswers.map((k) => (
                                <span
                                  ...
                                  style={{
                                    fontSize: 11,
                                    fontWeight: k === q.correctKey ? 800 : 600,
                                    color:
                                      k === q.correctKey
                                        ? "#4CAF50"
                                        : "#7D5A2C",
            ...
            >
              ...
              >
                {usersLoading ? (
                  ...
                  <LoadMsg msg="Chưa có giáo viên nào." />
                ) : (
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          ...
                          {u.username === currentUser && (
                            <span style={{ fontSize: 12, color: "#F5A800" }}>
                              {" "}
                              (Bạn)
                            </span>
                          )}
              ...
              >
                {menuLoading ? (
                  ...
                  <>
                    ...
                    >
                      Tuỳ chỉnh tên hiển thị của từng mục trong menu học sinh.
                      Để trống = dùng tên mặc định.
                    </div>
                      ...
                      >
                        ...
                        >
                          <div>
                            ...
                            <input
                              value={menuDraft[key]?.label ?? ""}
                              onChange={(e) =>
                                setMenuDraft((p) => ({
                                  ...p,
                                  [key]: { ...p[key], label: e.target.value },
                                }))
                              }
                          ...
                          <div>
                            ...
                            <input
                              value={menuDraft[key]?.sub ?? ""}
                              onChange={(e) =>
                                setMenuDraft((p) => ({
                                  ...p,
                                  [key]: { ...p[key], sub: e.target.value },
                                }))
                              }
                              placeholder={def.sub}
                              style={inputStyle}
